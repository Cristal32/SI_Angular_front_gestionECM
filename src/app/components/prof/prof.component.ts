...
export class ProfComponent {
  ...
  setFormInCreatedProf(){
    ...
    this.createdProf.dateEmb = this.form.dateEmbauche_prof;
    this.createdProf.dateDep = this.form.dateDepart_prof;
  }
  ...
  console.log("createdProf: ", this.createdProf);
...
deleteProfForm(){
  ...
  matchesSearchCriteria(prof: Prof, text: string): boolean {
    const searchFields: (string | undefined)[] = [
      ...
      prof.adrs.toString(),
      prof.tel.toString(),
      prof.dateEmb?.toString(),
      prof.dateDep?.toString()
      ];
